...
Fresh KMP, Compose, iOS simulator, Android API-36 emulator, HEVC round-trip, cleanup, demo-launch, Samsung `RZCX519T5FL` physical, and API-23 device evidence is recorded in `docs/verification/2026-08-06-android-hevc-release-gate.md`. The Samsung SM-S928B/API36 focused suite passed 17 tests; its HEVC production factory emitted a library-owned absolute cache path with `video/avc` + `audio/mp4a-latm`, then returned `Cleared` and left no file. The fresh `ClipEditor_API23` Android 6.0 suite passed 43 tests with zero XML failures/errors/skips. R1 verification evidence is complete; independent final review, not another device-evidence hold, remains before publication. The frozen contract and iOS typed-unavailable behavior remain unchanged.
