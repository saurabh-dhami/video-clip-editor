...
## R1 API-23 refresh — 2026-08-06

The API-23 prerequisite is now satisfied. `env ANDROID_HOME=/Users/sandeepdhami/Library/Android/sdk ANDROID_SERIAL=emulator-5554 ./gradlew :video-clip-editor-core:connectedAndroidDeviceTest --rerun-tasks` exited 0 with `Starting 43 tests on ClipEditor_API23(AVD) - 6.0`, `Finished 43 tests`, and `BUILD SUCCESSFUL in 14s`. The generated XML, `video-clip-editor-core/build/outputs/androidTest-results/connected/androidMain/TEST-ClipEditor_API23(AVD) - 6.0-_video-clip-editor-core-.xml`, records 43 tests, zero failures/errors/skips, and 6.692 s aggregate time; its instrumentation log records `OK (43 tests)` in 5.902 s.

The API23 HEVC production-factory log independently records an absolute library-owned output path, `video/avc`, `audio/mp4a-latm`, `cleanup=Cleared`, and `existsAfterCleanup=false`. This replaces the previous capacity hold. It is evidence only: no frozen declaration changed, and independent final review remains required.

## R1 final Samsung decoder evidence — 2026-08-06

After decoder repair `8391171`, the final Samsung SM-S928B/API36 focused run passed 20/20. It includes all three `Media3DecoderCapabilityPreflightTest` cases (unavailable HEVC and AVC decoder typed failures before session/export; normal open-session behavior with an available AVC decoder), the HEVC production-factory round trip, six integration tests, and ten temporary-lease tests. `docs/verification/evidence/2026-08-06-samsung-r1/` contains byte-identical tracked copies of the raw JUnit XML, instrumentation stream, and HEVC log, each with original build path and SHA-256 in its manifest.

The final HEVC raw line records an absolute owned cache path, `video/avc`, `audio/mp4a-latm`, `cleanup=Cleared`, and `existsAfterCleanup=false`. The current post-repair full API23 suite passed 46/46. This is evidence only and does not alter any frozen declaration above; independent final review remains required.
