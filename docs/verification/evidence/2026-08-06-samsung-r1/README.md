...
The retained XML uses the source artifact's CRLF line endings and the instrumentation stream retains its source blank line at EOF. `.gitattributes` marks exactly these three raw files as `-text whitespace=-trailing-space,-blank-at-eof`, so Git preserves their bytes and excludes the source-formatting bytes from whitespace validation. Normalizing either file would break its recorded hash. The authored Markdown evidence files pass the scoped whitespace check.
